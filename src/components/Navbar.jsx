...
export default function Example() {
  ...
  return (
    ...
    <div key={page.name} className="flow-root">
      {page.dropdown ? (
        <div>
          <p className="block p-2 font-medium text-gray-900">{page.name}</p>
          <ul className="ml-4 space-y-1 mt-2">
            {page.items.map(item => (
              <li key={item.name}>
                <a href={item.href} className="-m-2 block p-2 text-gray-500 hover:text-gray-700">
                  {item.name}
                </a>
              </li>
            ))}
            ...
            <div className="flex h-16 items-center justify-between">
              ...
              <PopoverGroup className="hidden lg:block lg:flex-1 lg:self-stretch">
                <div className="flex h-full space-x-8">
                  ...
                  {navigation.pages.map((page) => 
  page.dropdown ? (
    <Popover key={page.name} className="flex group relative">
      {({ open }) => (
        <>
          <div className="relative flex">
            <PopoverButton className="group relative z-10 flex items-center justify-center text-sm font-medium text-gray-700 transition-colors duration-200 ease-out hover:text-gray-800 data-open:text-indigo-600">
              {page.name}
              {/* Chevron icon */}
              <span className="ml-1 flex items-center transition-transform duration-200 group-hover:rotate-180">
                <svg 
          ...
          <div className="absolute left-0 top-full z-10 hidden w-40 group-hover:block hover:block">
